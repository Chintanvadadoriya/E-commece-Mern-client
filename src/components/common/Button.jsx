...
export function EditeOrder({ openModal }) {
    return (
...
export function EditeProduct({ updateProduct }) {
    return (
        <button onClick={updateProduct} className="mr-2 inline-block px-3 py-1 text-sm leading-normal text-blue-600 dark:text-blue-500 bg-transparent border border-blue-600 dark:border-blue-500 rounded-md cursor-pointer hover:bg-blue-600 dark:hover:bg-blue-500 hover:text-white dark:hover:text-white transition duration-300 ease-in-out">Edit</button>
    )
}
...
export function Update({ onUpdate, type, loading, Loader }) {
    return (
        <button onClick={onUpdate} type="button" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">{loading ? <Loader /> : type}</button>
    )
}